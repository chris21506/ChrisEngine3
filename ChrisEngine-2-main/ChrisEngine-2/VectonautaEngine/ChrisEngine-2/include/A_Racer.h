...
/**
 * @class ChrisEngine2
 * @brief Corredores (NPC/Jugador) que siguen un path, cuentan vueltas y soportan
 *        un offset de ángulo para alinear el sprite con la dirección de marcha.
 */
class ChrisEngine2 : public Actor {
public:
    /**
     * @brief Construye un corredor.
     * @param name Nombre del actor (p. ej., princesa.png, sonic.png, virdo.png, wario.png).
     * @param playerId Identificador opcional del jugador (0 = IA).
     */
    explicit ChrisEngine2(const std::string& name, int playerId = 0);
     ...
     * @note Para este juego, la pista es "pista de carreras.png".
     */
...
private:
    ...
    float lookaheadDistance = 60.f;   ///< Distancia de mirada hacia delante.
    float arriveRadius = 30.f;        ///< Umbral para cambiar de waypoint.
    ...
    bool m_crossedLastFrame = false;  ///< Evita contar varias veces la misma pasada.
